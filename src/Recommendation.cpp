...
* @return int the betweenness centrality for a certain node/movie
*/
int Recommendation::betweennessCentrality(int lookingFor)
{
  ...
  for (int i = 0; i < (int) adjacency_matrix.size(); i++)
  {
    ...
    {
      if (i != j && i != lookingFor) {
        // get the list of neighbors for those two movies on which we are operating
        auto& neighbors1 = adjacency_matrix[i];
        auto& neighbors2 = adjacency_matrix[j];
        // check if both neighbors' lists include the movie we are looking for (edge != 0)

        if (neighbors1[lookingFor] > 0 && neighbors2[lookingFor] > 0)
        {
          betweenness_central_scores.push_back(1);
        }
        else
        {
          double score = 0;
...
double Recommendation::getSimilarity(int idx1, int idx2) {
